#!/usr/bin/env python3
"""Create HNCode problems directly from a bundled local zip."""
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Upload new HNCode problems from a local zip package")
    parser.add_argument("zip_path", type=Path, help="Input package zip")
    parser.add_argument("--base-url", default=DEFAULT_BASE_URL)
    parser.add_argument("--username", default=os.getenv("HNCODE_USER", "hncode"))
    parser.add_argument("--password", default=os.getenv("HNCODE_PASS"))
    ...
    parser.add_argument("--no-submit", action="store_true", help="Accepted for UI symmetry; HNCode upload does not submit")
    parser.add_argument("--only", nargs="*", help="Optional problem codes to process")
    parser.add_argument("--out-dir", type=Path, default=Path("hncode_upload_artifacts"))
...
def main(argv: list[str]) -> int:
    ...
    require(args.password or args.dry_run, "Missing password. Set HNCODE_PASS or pass --password.")
    ...
    for bundle in bundles:
        ...
        print(f"Prepared {bundle.code}: {bundle.name}, {len(tests.input_files)} tests, {tests.zip_path}")
    ...
    for bundle in bundles:
        ...
        if args.skip_create:
            ...
            print(f"Creating HNCode problem {bundle.code}")
