"""File-backed progress/job store for long-running UI actions."""
...
def waiting_payload(job_id: str = "") -> dict[str, Any]:
    return {
        "job_id": job_id,
        "phase": "waiting",
        "done": 0,
...
def read_job(progress_dir: Path, job_id: str | None) -> dict[str, Any]:
    ...
    if not job_id:
        raise ValueError("invalid progress_id")
    path = job_path(progress_dir, job_id)
    if not path.exists():
        return waiting_payload(job_id)
    try:
        return json.loads(path.read_text(encoding="utf-8"))
    except Exception:
        data = waiting_payload(job_id)
        data["status"] = "failed"
        data["finished"] = True
        data["ok"] = False
        data["message"] = "Cannot read progress file."
        return data
