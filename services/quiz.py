...
def find_image_in_dir(filename: str, directory: Path) -> Path | None:
    for ext in ["", ".png", ".jpg", ".jpeg", ".gif"]:
        path = directory / f"{filename}{ext}"
        if path.is_file():
            return path
    for p in directory.rglob("*"):
        if p.is_file() and p.stem.lower() == filename.lower():
            return p
    return None

def upload_quiz_image(session, base_url: str, img_path: Path) -> str | None:
    upload_url = f"{base_url}/api/upload/image"
    try:
        with open(img_path, "rb") as f:
            r = session.post(upload_url, files={"image": f})
